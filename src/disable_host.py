...
print('[-] Select one to disable')
print('\n'.join('> ' + '\t'.join(s for s in content[i * 3 : i * 3 + 3]) for i in range(len(content) // 3 + 1)), end="\n\n")

target =  input('Please enter the hostname you want to disable: ')
mapping_dict[target].click()
browser.find_elements_by_class_name('btn-alt')[2].click()
